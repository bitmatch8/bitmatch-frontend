...
function isScriptPubKeyHexOpReturn(scriptPubKeyHex: string) {
  return (
    scriptPubKeyHex && Buffer.from(scriptPubKeyHex, "hex")[0] === OP_RETURN
  );
}
...
export type UpdaterTx = {
  txid: string;
  vin: (
    | { txid: string; vout: number; txinwitness: string[] }
    | { coinbase: string }
  )[];
...
export class RuneUpdater implements RuneBlockIndex {
  ...
  async indexRunes(tx: UpdaterTx, txIndex: number): Promise<void> {
    ...
    const allocated: Map<string, RuneBalance>[] = [
      ...new Array(tx.vout.length),
    ].map(() => new Map());
      ...
      if (isRunestone(artifact)) {
        ...
        for (const { id, amount, output } of [...runestone.edicts]) {
          ...
          if (output > tx.vout.length) {
            throw new Error(
              "Runestone edict output should never exceed transaction output size"
            );
          }
          ...
          let allocate = (amount: u128, output: number) => {
            if (amount > 0n) {
              const currentAllocated = getAllocatedRuneBalance(
                output,
                runeLocation
              );
              maybeBalance.amount = u128.checkedSubThrow(
                u128(maybeBalance.amount),
                amount
              );
              currentAllocated.amount = u128.checkedAddThrow(
                u128(currentAllocated.amount),
                amount
              );
            }
          };
          ...
          if (Number(output) === tx.vout.length) {
            ...
            const destinations = [...tx.vout.entries()]
              .filter(
                ([_, vout]) => !isScriptPubKeyHexOpReturn(vout.scriptPubKey.hex)
              )
              .map(([index]) => index);
            ...
            if (amount === 0n) {
              ...
              const amount = u128(
                u128(maybeBalance.amount) / u128(destinations.length)
              );
              const remainder =
                u128(maybeBalance.amount) % u128(destinations.length);
              ...
              for (const [i, output] of destinations.entries()) {
                allocate(
                  i < remainder
                    ? u128.checkedAddThrow(amount, u128(1))
                    : amount,
                  output
                );
              }
    ...
    if (optionArtifact.isSome() && !isRunestone(optionArtifact.unwrap())) {
      ...
      const optionVout = pointer
        ...
        .inspect((pointer) => pointer < allocated.length)
        .orElse(() => {
      ...
      if (optionVout.isSome()) {
        ...
        for (const balance of unallocated.values()) {
          if (balance.amount > 0) {
            const currentBalance = getAllocatedRuneBalance(
              vout,
              balance.runeId
            );
            currentBalance.amount = u128.checkedAddThrow(
    ...
    for (const [vout, balances] of allocated.entries()) {
      ...
      const etchingByRuneId = new Map(
        this.etchings.map((etching) => [
          RuneLocation.toString(etching.runeId),
          etching,
        ])
      );
      for (const balance of balances.values()) {
        ...
        const etching =
          ...
          (await this._storage.getEtching(runeIdString));
        if (etching === null) {
  ...
  async etched(
    ...
    if (optionRune.isSome()) {
      ...
      if (
        ...
        this.etchings.find(
          (etching) =>
            SpacedRune.fromString(etching.runeName).rune.toString() ===
            rune.toString()
        ) ||
  ...
  private async mint(id: RuneLocation, txid: string): Promise<Option<bigint>> {
    ...
    const etching =
      ...
      (await this._storage.getEtching(runeLocation));
    ...
    const startRelative =
      terms.offset?.start !== undefined
        ? etching.runeId.block + Number(terms.offset.start)
        : null;
    const startAbsolute =
      terms.height?.start !== undefined ? Number(terms.height.start) : null;
    const start =
    ...
    const endRelative =
      terms.offset?.end !== undefined
        ? etching.runeId.block + Number(terms.offset.end)
        : null;
    const endAbsolute =
      terms.height?.end !== undefined ? Number(terms.height.end) : null;
    const end =
    ...
    const currentBlockMints = this._mintCountsByRuneLocation.get(
      runeLocation
    ) ?? {
      mint: id,
      count: 0,
    ...
    const totalMints =
      ...
      (await this._storage.getValidMintCount(
        runeLocation,
  ...
  private async unallocated(tx: UpdaterTx) {
    ...
    for (const input of tx.vin) {
      ...
      for (const additionalBalance of utxoBalance) {
        ...
        balance.amount = u128.checkedAddThrow(
          u128(balance.amount),
          u128(additionalBalance.amount)
        );
      }
    }
  ...
  async txCommitsToRune(tx: UpdaterTx, rune: Rune): Promise<boolean> {
    ...
    for (const input of tx.vin) {
      ...
      const offset =
        ...
        lastWitnessElement[0] === TAPROOT_ANNEX_PREFIX
          ? 3
          : 2;
      ...
      for (const instruction of instructions) {
        ...
        const commitTxHeightResult = await this._rpc.getblock({
        ...
        const confirmations =
          u128.checkedSubThrow(u128(this.block.height), u128(commitTxHeight)) +
          1n;
  ...
  createEtching(
    ...
    if (isRunestone(artifact)) {
      const { divisibility, terms, premine, spacers, symbol } =
        artifact.etching.unwrap();
      this.etchings.push({
        valid: true,
        runeTicker: rune.toString(),
        runeName: new SpacedRune(
          rune,
          Number(spacers.map(Number).unwrapOr(0))
        ).toString(),
        runeId,
        txid,
        ...(divisibility.isSome()
          ? { divisibility: divisibility.map(Number).unwrap() }
          : {}),
              ...
              terms: (() => {
                ...
                return {
                  ...(unwrappedTerms.amount.isSome()
                    ...
                    : {}),
                  ...(unwrappedTerms.cap.isSome()
                    ? { cap: unwrappedTerms.cap.unwrap() }
                    : {}),
                  ...(unwrappedTerms.height.filter((option) => option.isSome())
                    .length
                    ? {
                        height: {
                    ...
                    : {}),
                  ...(unwrappedTerms.offset.filter((option) => option.isSome())
                    .length
                    ? {
                        offset: {
